...
const HistoryModule = (() => {
  ...
  function renderCategoryChips(allTx){
    const wrap = document.getElementById('histCatFilterWrap');
    if (!wrap) return;

    const cats = Array.from(new Set(allTx.map(t => t.category).filter(Boolean))).sort();
    if (!cats.length){ wrap.innerHTML = ''; return; }

    // hanya render ulang chip kalau daftar kategori berubah, supaya state 'selected' tidak hilang tiap render
    if (wrap.dataset.rendered === cats.join('|')) {
      wrap.querySelectorAll('.hist-cat-chip').forEach(chip => {
        chip.classList.toggle('active', selectedCategories.has(chip.dataset.cat));
      });
      return;
    }
    wrap.dataset.rendered = cats.join('|');

    wrap.innerHTML = cats.map(c => `
      <button type="button" class="hist-cat-chip ${selectedCategories.has(c) ? 'active' : ''}" data-cat="${Utils.escapeHtml(c)}">${Utils.escapeHtml(c)}</button>
    `).join('');
  ...
  function render(opts = {}){
    ...
    const activeCats = getActiveCategoryFilters();
    if (activeCats.length){
      list = list.filter(t => activeCats.includes(t.category));
    }

    renderCategoryChips(allTx);
