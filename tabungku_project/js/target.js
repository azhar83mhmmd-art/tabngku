...
const TargetModule = (() => {
  ...
  function render(){
    ...
    wrap.innerHTML = targets.map(t => {
      ...
      const perDay = (!t.achieved && daysLeft > 0) ? Math.ceil(sisa / daysLeft) : 0;
      const perWeek = (!t.achieved && daysLeft > 0) ? Math.ceil(sisa / (daysLeft / 7)) : 0;
      return `
        <div class="card target-card">
          ...
          ${(!t.achieved && daysLeft > 0) ? `
          <div class="target-calc">
            <div><span>Perlu nabung / hari</span><b>${Utils.formatRupiah(perDay)}</b></div>
            <div><span>Perlu nabung / minggu</span><b>${Utils.formatRupiah(perWeek)}</b></div>
          </div>` : ''}
          ${!t.achieved ? `
